...
def load_treasury_operations_state():
    ...
    try:
        response = s3_client.get_object(
            ...
            Key="new-york-fed/.state/treasury_operations_state.json"
        )
        return json.loads(response['Body'].read())
    except s3_client.exceptions.NoSuchKey:
        return {}

def save_treasury_operations_state(state_data):
...
async def process_treasury_operations_async():
    ...
    state = load_treasury_operations_state()
    ...
    if records:
        ...
        save_treasury_operations_state(state)
