...
def load_primary_dealer_stats_state():
    ...
    try:
        response = s3_client.get_object(
            ...
            Key="new-york-fed/.state/primary_dealer_stats_state.json"
        )
        return json.loads(response['Body'].read())
    except s3_client.exceptions.NoSuchKey:
        return {}

def save_primary_dealer_stats_state(state_data):
...
async def process_primary_dealer_stats_async():
    ...
    state = load_primary_dealer_stats_state()
    ...
    save_primary_dealer_stats_state(state)
