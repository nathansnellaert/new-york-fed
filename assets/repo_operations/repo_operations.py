...
def load_repo_operations_state():
    ...
    try:
        response = s3_client.get_object(
            ...
            Key="new-york-fed/.state/repo_operations_state.json"
        )
        return json.loads(response['Body'].read())
    except s3_client.exceptions.NoSuchKey:
        return {}

def save_repo_operations_state(state_data):
...
async def fetch_repo_data(client, endpoint):
    """Fetch repo data from NY Fed API"""
...
def parse_integer(value):
    """Parse integer value, handling None and empty strings"""
...
async def fetch_historical_operations(client, start_date, end_date):
    """Fetch historical repo operations data"""
    records = []
    ...
    while current_start <= end_date:
        ...
        print(f"  Fetching repo operations for {params['startDate']} to {params['endDate']}")
        
        # Fetch operations data using the correct endpoint
        data = await fetch_repo_data(client, f"rp/results/search.json?startDate={params['startDate']}&endDate={params['endDate']}")
        
        if data and "repo" in data and "operations" in data["repo"]:
            for operation in data["repo"]["operations"]:
                # Skip if no results
                ...
                # Process each detail (security type) within the operation
                ...
                if details:
                    ...
                    # Create single record for operation without details
                    record = {
                        "operation_date": operation_date,
                        "operation_id": operation_id,
                        "operation_type": operation_type,
...
async def process_repo_operations_async():
    """Process repo operations data"""
    state = load_repo_operations_state()
    ...
    print(f"Fetching repo operations from {start_date} to {end_date}")
    ...
    if records:
        ...
        save_repo_operations_state(state)
        
        print(f"Fetched {len(records)} repo operation records")
        return pa.Table.from_pylist(records, schema=schema)
    
    return pa.Table.from_pylist([], schema=schema)

def process_repo_operations():
    """Synchronous wrapper for async processing"""
    return asyncio.run(process_repo_operations_async())