...
def load_securities_lending_state():
    ...
    try:
        response = s3_client.get_object(
            ...
            Key="new-york-fed/.state/securities_lending_state.json"
        )
        return json.loads(response['Body'].read())
    except s3_client.exceptions.NoSuchKey:
        return {}

def save_securities_lending_state(state_data):
...
async def process_securities_lending_async():
    ...
    state = load_securities_lending_state()
    ...
    if records:
        ...
        save_securities_lending_state(state)
