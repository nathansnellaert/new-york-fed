import os
os.environ['CONNECTOR_NAME'] = 'new-york-fed'
...
def main():
    ...
    upload_data(reference_rates_data, "reference_rates")
    
    soma_holdings_data = process_soma_holdings()
    upload_data(soma_holdings_data, "soma_holdings")
    
    primary_dealer_data = process_primary_dealer_stats()
    upload_data(primary_dealer_data, "primary_dealer_stats")
    
    ambs_data = process_ambs_operations()
    upload_data(ambs_data, "ambs_operations")
    
    treasury_data = process_treasury_operations()
    upload_data(treasury_data, "treasury_operations")
    
    seclending_data = process_securities_lending()
    upload_data(seclending_data, "securities_lending")
    
    fx_swaps_data = process_fx_swaps()
    upload_data(fx_swaps_data, "fx_swaps")
    
    repo_data = process_repo_operations()
    upload_data(repo_data, "repo_operations")
